...
async function tune(id) {
  ...
  console.log(`\n${config.name} (${id})`);
  console.log("  tol   raw   kept  lifts   runs  valleys  vertical        pieces  usable");
  console.log("  ----  ----  ----  -----  -----  -------  --------------  ------  ------");
  ...
  for (const tolerance of STEPS) {
    ...
    console.log(
      ...
      `${String(`${row.runs}/${row.rawRuns}`).padStart(5)}  ${String(row.valleys).padStart(7)}  ` +
  ...
  // Valleys first, then vertical, then lifts. A tie goes to the smaller
  // tolerance, since merging two places that are not the same place is the
  ...
  const best = usable.reduce((a, b) => {
    ...
    if (a.lifts !== b.lifts) return a.lifts > b.lifts ? a : b;
  ...
  console.log(`    ${best.valleys} valley(s) you can start a day in, ` +
    `${best.high - best.low} m of vertical, ${best.lifts} lifts, ${best.runs} runs`);
